...
/*******************************************************************************
** ATCR_WooCommerce_Handler
** Handler for WooCommerce
** @since 1.0
*******************************************************************************/
class ATCR_WooCommerce_Handler extends ATCR_Handler {
    ...
    *******************************************************************************/
    public function __construct() {
        ...
        add_action('woocommerce_add_to_cart', array($this, 'detectAddToCart'), 10, 6);
    }
    ...
    ** @param $cart_item_key key for the item in the cart
    ** @param $quantity quantity that was added to the cart
    ** @param $variation_id id of the variation (if a variable product)
    ** @param $variation variation data
    ** @param $cart_item_data item data
    ** @since 1.0
    *******************************************************************************/
    function detectAddToCart($cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data) {
        $requestURI = trailingslashit(atcrFilterData($_SERVER['HTTP_REFERER']));
        $productURL = trailingslashit(get_permalink($product_id));
        ...
        if (strcasecmp($requestURI, $productURL) === 0) {
            ...
            $this->recordAddToCart($product_id);
        }
    }
    ...
    *******************************************************************************/
    function addColumnToProductList() {
        add_filter('manage_product_posts_columns', array($this, 'filterProductColumns'), 10, 1);
    	add_action('manage_product_posts_custom_column', array($this, 'showColumnInList'), 10, 1);
        add_filter('manage_edit-product_sortable_columns', array($this, 'sortColumnInList'));
